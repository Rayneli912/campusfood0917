...
export function useRecentViews() {
  ...
  const loadBackend = useCallback(async () => {
    try {
      setLoading(true)
      const r = await fetch(apiUrl("/api/recent-views"), { method: "GET" })
  ...
  const addBackend = async (payload: Omit<RecentView, "viewedAt">) => {
    ...
    const uid = readUserId()
    if (uid) notifyDataUpdate(RECENT_VIEWS_UPDATED, { userId: uid, recentViews: items })
  }

  const clearBackend = async () => {
    const r = await fetch(apiUrl("/api/recent-views"), { method: "DELETE" })
    if (!r.ok) throw new Error("clear recent views failed")
    setViews([])
    window.dispatchEvent(new CustomEvent("recentViewsUpdated", { detail: { items: [] } }))
    const uid = readUserId()
    if (uid) notifyDataUpdate(RECENT_VIEWS_UPDATED, { userId: uid, recentViews: [] })
  }

  const add = useCallback(async (payload: Omit<RecentView, "viewedAt">) => (USE_BACKEND ? addBackend(payload) : addLocal(payload)), [views])
  const clear = useCallback(async () => (USE_BACKEND ? clearBackend() : clearLocal()), [views])
