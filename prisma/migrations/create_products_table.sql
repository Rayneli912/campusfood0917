...
-- RLS 策略：允許店家管理自己的商品
DROP POLICY IF EXISTS "Allow stores to manage their products" ON products;
CREATE POLICY "Allow stores to manage their products" ON products
  FOR ALL USING (
    store_id IN (SELECT id FROM stores WHERE id = auth.uid())
  );
