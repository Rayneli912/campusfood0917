...
function parseLooseKeyValues(raw: string): ParsedFields {
  ...
  for (let i = 0; i < hits.length; i++) {
    ...
    if (!key) continue
    if (key === "quantity") {
      const { n, unit } = extractQuantityAndUnit(value)
      if (typeof n === "number" && n > 0) {
        fields.quantity = n
        if (unit) fields.quantityUnit = unit
      }
    } else {
...
export function toCanonicalContent(fields: Required<Pick<ParsedFields, "location" | "item" | "quantity" | "deadline">> & { note?: string; quantityUnit?: string }) {
  const qtyText = `${fields.quantity}${fields.quantityUnit ?? ""}`
  return [
    `【地點】：${fields.location}`,
    `【物品】：${fields.item}`,
    `【數量】：${qtyText}`,
...
export class LineMessageParser {
  ...
  static parseNewPostMessage(message: string): {
    ...
    for (const k of REQUIRED_KEYS) {
      if (k === "quantity") {
        if (!(fields.quantity && fields.quantity > 0)) missing.push("數量")
      } else if (!fields[k]) {
        missing.push(k === "location" ? "地點" : k === "item" ? "物品" : "領取期限")
      }
    }
    if (missing.length > 0) {
