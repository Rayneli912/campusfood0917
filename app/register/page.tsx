...
export default function RegisterPage() {
  ...
  const handleUserRegister = (e: React.FormEvent) => {
    ...
    // 從 localStorage 中獲取已註冊用戶
    const registeredUsers = JSON.parse(localStorage.getItem("registeredUsers") || "[]")

    // 檢查帳號是否已存在
    if (registeredUsers.some((user: any) => user.username === userForm.username)) {
      setUserErrors((prev) => ({ ...prev, username: "該帳號已被使用" }))
      return
    }
    ...
    const newUser = {
      ...
      department: userForm.department, // 儲存系級資料
  ...
  const handleStoreRegister = (e: React.FormEvent) => {
    ...
    // 從 localStorage 中獲取已註冊店家
    const registeredStores = JSON.parse(localStorage.getItem("registeredStores") || "[]")

    // 檢查帳號是否已存在
    if (registeredStores.some((store: any) => store.username === storeForm.username)) {
      setStoreErrors((prev) => ({ ...prev, username: "該帳號已被使用" }))
      return
    }
    ...
    // 更新 localStorage
    localStorage.setItem("registeredStores", JSON.stringify([...registeredStores, newStore]))
