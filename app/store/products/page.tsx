...
export default function StoreProductsPage() {
  ...
  const uploadImage = async (file: File): Promise<string | null> => {
    try {
      ...
      toast({
        title: "錯誤",
        description: error.message || "圖片上傳失敗",
        variant: "destructive",
      })
      return null
    } finally {
  ...
  const handleEditProduct = (product: Product) => {
    ...
    setShowProductDialog(true) // ✅ 直接打開 Dialog
  }
  ...
  const handleSubmitProduct = async () => {
    ...
    // ✅ 驗證必填欄位（包含期限）
    if (!productForm.name || !productForm.discount_price || !productForm.quantity || !productForm.expiry_date) {
      toast({
        title: "錯誤",
        description: "請填寫所有必填欄位（商品名稱、特價、數量、期限）",
        variant: "destructive",
      })
      return
    }
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {products.map((product) => (
                <Card 
                  key={product.id}
                  className={!product.is_available ? "opacity-50" : ""} // ✅ 下架商品半透明
                >
                  <CardHeader>
      ...
      <Dialog open={showProductDialog} onOpenChange={setShowProductDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="image">商品圖片 {uploadingImage && <span className="text-sm text-blue-600">上傳中...</span>}</Label>
                <Input
                  id="image"
                  type="file"
                  accept="image/*"
          ...
          <DialogFooter className="flex gap-2">
            ...
            <Button onClick={handleSubmitProduct} disabled={submitting || uploadingImage} className="flex-1">
              {submitting ? (
