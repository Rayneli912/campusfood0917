...
import CountersPanel from "@/components/admin/counters-panel"
...
export default function AdminDashboardPage() {
  ...
  const loadStats = () => {
    try {
      ...
      const stores = JSON.parse(localStorage.getItem("registeredStores") || "[]")
      const orders = JSON.parse(localStorage.getItem("orders") || "[]")
      const pendingStores = JSON.parse(localStorage.getItem("pendingStores") || "[]")

      const activeUsers = users.filter((user: any) => !user.isDisabled).length
  ...
  return (
    <div className="p-4 md:p-8">
      <h1 className="text-2xl md:text-3xl font-bold tracking-tight mb-4 md:mb-6">管理員儀表板</h1>
